...
def check(ip, port=445):
    ...
    try:
        ...
        if nt_status == '\x05\x02\x00\xc0':
            result = "[+] [{}] is likely VULNERABLE to MS17-010! ({})".format(ip, native_os)
            # log.info(result)
...
def host_scan(target_ips):
    result_dict = {}
    try:
        if re.search('/', target_ips):  # 网段
            hosts = ipaddr.IPv4Network(target_ips).iterhosts()
            for IP in hosts:
                result = check(str(IP))
                if result:
                    result_dict[str(IP)] = result
        else:
            result = check(target_ips)
            if result:
                result_dict[target_ips] = result
    except Exception as err:
        print ("IP error:{}".format(err))
    return result_dict


if __name__ == '__main__':
    target = '192.168.5.158/25'
    host_scan(target_ips=target)
