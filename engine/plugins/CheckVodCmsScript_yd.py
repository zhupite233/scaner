...
def run_url(http, config, item):
    ...
    try:
        ...
        if response['httpcode'] == 200:
            if response['response_body'].find('phpvod') != -1:
                injectInfo = returnInjectResult(url=new_url, confirm=1, detail="检测到phpvod/vodcms 视频管理系统", response=response)
                responseList.append(getRecord2(scanInfo, injectInfo))
                return responseList

    except Exception, e:
        logger.exception("CheckVodCmsScript_yd" + e.message)
    return responseList
